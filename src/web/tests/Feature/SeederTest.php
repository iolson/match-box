...
it('seeds active duty maps as active and reserves as inactive', function () {
    $this->seed(\Database\Seeders\MapSeeder::class);

    expect(Map::where('is_active', true)->count())->toBe(7);
    expect(Map::where('is_active', false)->count())->toBe(2);
    expect(Map::where('name', 'de_train')->value('is_active'))->toBeFalse();
    expect(Map::where('name', 'de_vertigo')->value('is_active'))->toBeFalse();
});
